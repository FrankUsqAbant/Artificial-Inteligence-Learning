...
const PeriodicTable: React.FC<PeriodicTableProps> = ({
  ...
  const activeCategory =
    ...
    (unlockedCategories.length > 0
      ? unlockedCategories[unlockedCategories.length - 1]
      : "math-foundation");
  const categoryTheme = CATEGORY_THEMES[activeCategory];

  const filteredElements = useMemo(() => {
    const term = search.toLowerCase();
    return AI_ELEMENTS.filter(
      (el) =>
  ...
  const masteryPercentage = Math.round(
    (masteredSymbols.length / AI_ELEMENTS.length) * 100
  );
  ...
  return (
    <div className="p-4 md:p-8 min-h-screen bg-[#050505] flex flex-col relative overflow-hidden">
      {/* Context Top Bar */}
      <div
        className={`fixed top-0 left-0 right-0 z-50 p-4 border-b ${categoryTheme.border} bg-black/60 backdrop-blur-md transition-all duration-500`}
      >
        <div className="max-w-[1400px] mx-auto flex items-center justify-between">
          <div className="flex items-center gap-6">
            <span
              className={`text-[10px] font-mono uppercase tracking-[0.5em] px-3 py-1 bg-slate-900 border ${categoryTheme.border} ${categoryTheme.color}`}
            >
              Learning_Phase: {activeCategory.replace("-", "_")}
            </span>
            <p className="text-[11px] text-slate-400 font-mono hidden md:block">
              <span className="text-white font-bold mr-2">CONTEXT:</span>{" "}
              {categoryTheme.desc}
            </p>
          </div>
          ...
          >
            {filteredElements.map((el) => {
              ...
              return (
                ...
                >
                  <span
                    className={`absolute top-1 left-1 text-[8px] font-mono ${
                      isMastered ? theme.color : "text-slate-700"
                    }`}
                  >
                    {el.atomic_number}
                  </span>
                  <span
                    className={`text-2xl font-black italic tracking-tighter ${
                      isMastered ? theme.color : "text-slate-500"
                    }`}
                  >
                    {el.symbol}
                  </span>
                  <span
                    className={`text-[8px] font-bold uppercase truncate w-full px-1 text-center ${
                      isMastered ? "text-slate-300" : "text-slate-700"
                    }`}
                  >
                    {isLocked ? "LOCKED" : el.name}
                  </span>
                </button>
              );
            })}
